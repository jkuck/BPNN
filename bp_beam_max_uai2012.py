...
from uai_2012.uai2012_models import UAI2012Dataset
from uai_2012.libdai_utils import run_map_loopyBP, logScore, build_libdaiFactorGraph_from_UAI2012Model
import random
...
parser.add_argument('--max_search_step', type=int, default=100)
...
MAX_SEARCH_STEP = args.max_search_step

metric_names = [
    'score_relative', 'score_mse', 'score_rmse', 'score_abs',
    'logscore_relative', 'logscore_mse', 'logscore_rmse', 'logscore_abs',
]
ratio_list = [0, 0.01, 0.1, 0.2, 0.3, 0.5, 0.7, 0.8, 0.9, 0.99, 1]
snapshot_step = [int(ratio*MAX_SEARCH_STEP+0.5) for ratio in ratio_list]
expanded_metric_names = [mn+'_%d'%(st) for st in snapshot_step for mn in metric_names]
...
##########################
if USE_WANDB:
    wandb.init(project="BP_beam_max_UAI2012")
    ...
    wandb.config.BEAM_SIZE = BEAM_SIZE
    wandb.config.MAX_SEARCH_STEP = MAX_SEARCH_STEP
...
def _loss_func(bp_state, pred_state, factor_graph, metric_names):
    bp_logScore = logScore(factor_graph, bp_state)
    pred_logScore = logScore(factor_graph, pred_state)
    bp_score = np.exp(bp_logScore)
    pred_score = np.exp(pred_logScore)
    loss = []
    for mn in metric_names:
        y, p = None, None
        if 'logscore' in mn:
            y, p = bp_logScore, pred_logScore
        elif 'score' in mn:
            y, p = bp_score, pred_score
        else:
        ...
        if 'relative' in mn:
            ...
            loss.append(np.mean(p))
        else:
            raise ValueError("Wrong metric name: ", mn)
    return loss

def loss_func(bp_state, pred_state, factor_graph, metric_names, var_cards):
    loss, beam_size, max_search_step = [], BEAM_SIZE, MAX_SEARCH_STEP
    # update the pred_state using beam_search
    search_cache = [pred_state, ]
    cache_scores = np.array([logScore(factor_graph, pred_state)])
    for step in range(max_search_step+1):
        if step in snapshot_step:
            losses = _loss_func(bp_state, search_cache[0], factor_graph, metric_names)
            loss += losses
        if step < max_search_step:
            tmp_cache = [(state[:idx] if idx > 0 else tuple())+(np.random.choice(var_cards[idx]), )+(state[idx+1:] if idx < len(state)-1 else tuple())
                         for state in search_cache for idx in range(len(state))] + search_cache[::-1]
            tmp_cache = list(set(tmp_cache))
            tmp_scores = np.array([logScore(factor_graph, state) for state in tmp_cache])
            tmp_indexes = np.argsort(tmp_scores)[::-1][:beam_size]
            tmp_scores = tmp_scores[tmp_indexes]
            if np.all(cache_scores == tmp_scores):
                losses = _loss_func(bp_state, search_cache[0], factor_graph, metric_names)
                for step in range(step+1, max_search_step+1):
                    if step in snapshot_step:
                        loss += losses
                return loss
            cache_scores = tmp_scores
            search_cache = [tmp_cache[idx] for idx in tmp_indexes]
    return loss
...
# lbp_net.double()
def train():
    raw_data_list = UAI2012Dataset()
    random.shuffle(raw_data_list)
    raw_data_list_train, raw_data_list_val = raw_data_list[:int(len(raw_data_list)*0.7)], raw_data_list[int(len(raw_data_list)*0.7):]
    print('dataset size:', len(raw_data_list), len(raw_data_list_train), len(raw_data_list_val))
    ...
    for e in range(EPOCH_COUNT):
        epoch_loss, losses = 0, []
        for data in raw_data_list_train:
            fg = build_libdaiFactorGraph_from_UAI2012Model(data[0])
            best_bp_map_states = data[1]
            var_cards = [int(c.strip()) for c in data[0][2].strip().split()]

            bp_map_states = run_map_loopyBP(fg, map_flag=MODEL_MAP_FLAG, updates=UPDATES, damping=DAMPING,)
            loss = loss_func(
                best_bp_map_states,
                bp_map_states,
                fg,
        ...
        aggregated_losses = aggregate_loss_func(losses, expanded_metric_names)
        for mn, l in zip(expanded_metric_names, aggregated_losses):
            print(mn, ' = ', l)

        val_losses = []
        for data in raw_data_list_val:
            fg = build_libdaiFactorGraph_from_UAI2012Model(data[0])
            best_bp_map_states = data[1]
            var_cards = [int(c.strip()) for c in data[0][2].strip().split()]
        ...
        val_aggregated_losses = aggregate_loss_func(losses, expanded_metric_names)
        for mn, l in zip(expanded_metric_names, val_aggregated_losses):
            print(mn, ' = ', l)

        print()
        if USE_WANDB:
            train_logs = {mn+'_train':l for mn, l in zip(expanded_metric_names, aggregated_losses)}
            val_logs = {mn+'_val':l for mn, l in zip(expanded_metric_names, val_aggregated_losses)}
