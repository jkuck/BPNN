...
jobs = [
    'python learn_bp2_map_spinglass.py --append_bp_flag %s %s %s %s %s -lr %f --alpha %f --layer_num %d'%(
        dflag, aflag, mflag, sflag, bflag,
        10**(random.random()*4-5), damping, layer_num,
    )
...
for i in range(PROCESS_NUM):
    fname = 'bpnn_logz_converge_jobs_%d.sh'%i
