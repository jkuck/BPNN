...
jobs = [
    'python learn_bp_map_spinglass_nosym.py --no_training_flag %s %s %s %s --alpha %f --layer_num %d -C %f --C_upper_scale %f'%(
        dflag, aflag, mflag, bflag,
        damping, layer_num, C, c_upper_scale,
    )
...
for i in range(PROCESS_NUM):
    fname = 'bpnn_logz_nosym_notrain_jobs_%d.sh'%i
