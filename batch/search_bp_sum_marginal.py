    ...
    for damping in [0.0, 0.001, 0.01, 0.1, 0.3, 0.5, 0.7, 0.9, 0.99, 0.999]:
        for mflag in ['--model_map_flag', '']:
            for aflag in ['--attractive_flag', '']:
                for miter in [10000, 5, 10, 20, 100, 200, 500,]:
                    jobs.append('python bp_map_marginal_spinglass.py --data_map_flag %s %s --updates %s --damping %f --maxiter %d'%(aflag, mflag, updates, damping, miter))
...
for i in range(PROCESS_NUM):
    fname = 'bp_sum_marginal_jobs_%d.sh'%i
