...
def build_libdaiFactorGraph_from_SpinGlassModel(sg_model, fixed_variables={}):
    ...
    for var_idx in range(N**2):
        ...
        c = var_idx%N
        if r < N-1:
            factors.append(build_pairwise_factor(variables, fixed_variables, var_idx1=var_idx, var_idx2=var_idx+N, c=sg_model.cpl_params_v[r,c], c_upper_scale=sg_model.cpl_params_v_upper_scale[r,c] if 'cpl_params_v_upper_scale' in sg_model.__dict__ else 1.))
        if c < N-1:
            factors.append(build_pairwise_factor(variables, fixed_variables, var_idx1=var_idx, var_idx2=var_idx+1, c=sg_model.cpl_params_h[r,c], c_upper_scale=sg_model.cpl_params_h_upper_scale[r,c] if 'cpl_params_h_upper_scale' in sg_model.__dict__ else 1.))
