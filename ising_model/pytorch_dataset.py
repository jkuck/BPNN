    ...
    for row_idx in range(N):
        for col_idx in range(N-1):
            ...
            factor_potential, mask = build_pairwise_factor(
                ...
                c_upper_scale=sg_model.cpl_params_h_upper_scale[row_idx,col_idx] if 'cpl_params_h_upper_scale' in sg_model.__dict__ else 1.,
            )
            if transpose_flag:
    ...
    for row_idx in range(N-1):
        for col_idx in range(N):
            ...
            factor_potential, mask = build_pairwise_factor(
                ...
                state_dimensions=state_dimensions,
                c_upper_scale=sg_model.cpl_params_v_upper_scale[row_idx,col_idx] if 'cpl_params_v_upper_scale' in sg_model.__dict__ else 1.,
            )
            if transpose_flag:
                factor_potentials_list.append(factor_potential.transpose(0,1))
                masks_list.append(mask.transpose(0,1))
...
def build_factorgraph_from_SpinGlassModel_max(sg_model, idx=None, transpose_flag=False,):
    '''
    Convert a spin glass model to a factor graph pytorch representation
    ...
    ln_Z = sg_model.junction_tree_libdai(map_flag=True)
    marginals = None

    factor_graph = FactorGraphData(factor_potentials=factor_potentials,
                 factorToVar_edge_index=factorToVar_edge_index.t().contiguous(), numVars=num_vars, numFactors=num_factors,
                 edge_var_indices=edge_var_indices, state_dimensions=state_dimensions, factor_potential_masks=factor_potential_masks,
                 ln_Z=ln_Z, marginals=marginals, factorToVar_double_list=factorToVar_double_list, idx=idx,)
