...
#                     print("cur_loss between", val, "and", exact_ln_partition_function.float()[idx], "is:", cur_loss)
        ...
        if e % VAL_FREQUENCY == 0:
            ...
            if np.mean(graph_state_accuracies) >= 0.99:
                break;
    ...
    for spin_glass_problem in test_data_loader_pytorchGeometric: #pytorch geometric form
        ...
        for midx in range(model_num):
            ...
            mse_diff_log_loss, l1_diff_log_loss, \
                ...
                var_state_accuracy, graph_state_accuracy,\
                mse_logscore_state_loss, l1_logscore_state_loss\
                = test_loss_func(x, y, spin_glass_models_list_test[model_index+midx])
            mse_diff_log_losses += mse_diff_log_loss
            l1_diff_log_losses += l1_diff_log_loss
            mse_prob_losses += mse_prob_loss
            l1_prob_losses += l1_prob_loss
        ...
        model_index += model_num
    print("test accuracy =", np.mean(var_state_accuracies))
    print()
    if USE_WANDB:
        wandb.log({
            "RMSE_DiffLog_test": np.sqrt(np.mean(mse_diff_log_losses)),
            "L1_DiffLog_test": np.mean(l1_diff_log_losses),
            'RMSE_Prob_test': np.sqrt(np.mean(mse_prob_losses)),
            'L1_Prob_test': np.mean(l1_prob_losses),
            'CrossEntropy_Prob_test': np.mean(cross_entropy_prob_losses),
            'ACC_VarState_test': np.mean(var_state_accuracies),
            'ACC_GraphState_test': np.mean(graph_state_accuracies),
            'RMSE_LogScore_test': np.sqrt(np.mean(mse_logscore_state_losses)),
            'L1_LogScore_test': np.mean(l1_logscore_state_losses),
        })
