...
def build_factorgraph_from_sbm(sbm_model):
    ...
    mask_un = np.ones_like(un_factors)
    mask_un[0] = 0
    ...
    factor_mask_bi = np.zeros_like(factors)
    edgeToFacBi = np.repeat(np.arange(sbm_model.edge_index.shape[1]), 2) + sbm_model.N
    edgeToVarBi = np.ones(edge_index.shape[1] * 2)
    edgeToVarBi[0::2] = edge_index[1]
    edgeToVarBi[1::2] = edge_index[0]
    ...
    unFacVarLst = np.arange(sbm_model.N)[:, np.newaxis]
    binFacVarLst = sbm_model.edge_index.T
    factorToVar = np.concatenate((unFacVarLst, binFacVarLst), axis = 0).tolist()
    ...
    factor_graph = FactorGraphData(factor_potentials = torch.Tensor(factor_potentials),
                 factorToVar_edge_index = torch.Tensor(factorVarEdgeIndex), numVars = sbmModel.N, numFactors = factor_potentials.shape[0], 
    ...
    return factor_graph
sbm = StochasticBlockModel(100, .1, .1, 2)
print(sbm.edge_index.shape, sbm.gt_variable_labels)
