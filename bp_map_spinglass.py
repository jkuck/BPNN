...
##########################
if USE_WANDB:
    wandb.init(project="BP_map_spinGlass_reproduce")
...
#     with autograd.detect_anomaly():
    for e in range(EPOCH_COUNT):
        ...
        for spin_glass_problem in spin_glass_models_list_train:
            exact_ln_partition_function = spin_glass_problem.junction_tree_libdai(map_flag=DATA_MAP_FLAG)
            estimated_ln_partition_function = spin_glass_problem.loopyBP_libdai(
                map_flag=MODEL_MAP_FLAG, updates=UPDATES, damping=DAMPING,
            )

            loss = loss_func(torch.tensor(estimated_ln_partition_function).reshape(-1),
                             torch.tensor(exact_ln_partition_function).reshape(-1))
            epoch_loss += loss
