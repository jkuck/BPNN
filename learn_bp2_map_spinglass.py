...
parser.add_argument('--append_bp_flag', action='store_true', default=False)
...
APPEND_BP_FLAG = args.append_bp_flag
...
##########################
if USE_WANDB:
    wandb.init(project="learnBP2_map_spinGlass")
...
#     with autograd.detect_anomaly():
    for e in range(EPOCH_COUNT):
        epoch_loss = 0
        optimizer.zero_grad()
        losses, converged_flags, lower_bound_flags = [], [], []
        count = 0
...
#             spin_glass_problem.facToVar_edge_idx = spin_glass_problem.edge_index #hack for batching, see FactorGraphData in factor_graph.py
            ...
            estimated_ln_partition_function, converged = lbp_net(spin_glass_problem, return_converged=True)
...
#                     print("cur_loss between", val, "and", exact_ln_partition_function.float()[idx], "is:", cur_loss)
            ...
            lower_bound_flags += (estimated_ln_partition_function.squeeze() <= exact_ln_partition_function.float()).float().detach().cpu().tolist()
        ...
        if e % PRINT_FREQUENCY == 0:
            ...
            print("maximum difference training =", np.mean(converged_flags))
            print("lower bound ratio training = ", np.mean(lower_bound_flags))

        if e % VAL_FREQUENCY == 0:
            with torch.no_grad():
                val_losses, val_converged_flags, val_lower_bound_flags = [], [], []
    ...
    #                 print("loss:", loss)
                    ...
                    val_converged_flags.append(float(converged))
                    val_lower_bound_flags += (estimated_ln_partition_function.squeeze() <= exact_ln_partition_function.float()).float().detach().cpu().tolist()
                print("root mean squared validation error =", np.sqrt(np.mean(val_losses)))
                print("val maximum difference =", np.mean(val_converged_flags))
                print("lower bound ratio validation = ", np.mean(val_lower_bound_flags))

                val_transpose_losses, transpose_converged_flags, val_transpose_lower_bound_flags = [], [], []
    ...
    #                 print("loss:", loss)
                    ...
                    val_transpose_lower_bound_flags += (estimated_ln_partition_function.squeeze() <= exact_ln_partition_function.float()).float().detach().cpu().tolist()
                    transpose_converged_flags.append(float(converged))
                print("root mean squared validation error (transpose) =", np.sqrt(np.mean(val_transpose_losses)))
                print("val maximum difference (transpose) =", np.mean(transpose_converged_flags))
                print("lower bound ratio validation (transpose) = ", np.mean(val_transpose_lower_bound_flags))
                print()
                if USE_WANDB:
                    wandb.log({
                        "RMSE_val": np.sqrt(np.mean(val_losses)),
                        "RMSE_val_transpose": np.sqrt(np.mean(val_transpose_losses)),
                        "RMSE_training": np.sqrt(np.mean(losses)),
                        'MaxDiff_val': np.mean(val_converged_flags),
                        'MaxDiff_val_transpose': np.mean(transpose_converged_flags),
                        'MaxDiff_training': np.mean(converged_flags),
                        'LowerRatio_val': np.mean(val_lower_bound_flags),
                        'LowerRatio_val_transpose': np.mean(val_transpose_lower_bound_flags),
                        'LowerRatio_training': np.mean(lower_bound_flags),
                    })
        else:
            if USE_WANDB:
                wandb.log({
