...
def _loss_func(true_state, pred_state, sp_problem, metric_names):
...
def loss_func(true_state, pred_state, sp_problem, metric_names,):
    ...
    cache_scores = np.array([sp_problem.logScore(pred_state)])
    for step in range(max_search_step+1):
        if step in snapshot_step:
            losses = _loss_func(true_state, search_cache[0], sp_problem, metric_names)
            # loss_dict.update({(mn+'_%d'%step):l for mn,l in zip(metric_names, losses)})
            loss += losses
        if step < max_search_step:
            tmp_cache = [(state[:idx] if idx > 0 else tuple())+(1-state[idx],)+(state[idx+1:] if idx < len(state)-1 else tuple())
                         for state in search_cache for idx in range(len(state))] + search_cache[::-1]
            tmp_cache = list(set(tmp_cache))
            tmp_scores = np.array([sp_problem.logScore(state) for state in tmp_cache])
