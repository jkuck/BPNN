...
from permutation_equiv_utils_finalBethe import var_idx_perm_equivariant_2dfactor_all
    ...
    def __init__(self, max_factor_state_dimensions, msg_passing_iters, device=None, share_weights=SHARE_WEIGHTS,
        ...
        if bethe_MLP:
            ...
            if initialize_to_exact_bethe:  
                ...
                weight_initialization[0,-num_ones:] = torch.rand(num_ones)/belief_repeats
...
#         print("factor_graph.facToVar_edge_idx.shape:", factor_graph.facToVar_edge_idx.shape)
    ...
    #                                       prv_factorToVar_messages=factor_graph.prv_factorToVar_messages, prv_factor_beliefs=factor_graph.prv_factor_beliefs) 
                if self.bethe_MLP:
                    cur_pooled_fac, cur_pooled_var = self.compute_bethe_free_energy_pooledStates_MLP(factor_beliefs=prv_factor_beliefs, var_beliefs=prv_var_beliefs, factor_graph=factor_graph)
                    pooled_fac_beliefs.append(cur_pooled_fac)
                    pooled_var_beliefs.append(cur_pooled_var)
        ...
        if self.bethe_MLP:
            #print(torch.stack(pooled_fac_beliefs, dim = 1), torch.stack(pooled_var_beliefs, dim = 1))
            #time.sleep(10000)
            fac_mlp_output = var_idx_perm_equivariant_2dfactor_all(mlp=self.final_mlp_factors, input_tensor=torch.stack(pooled_fac_beliefs, dim=1))
            var_mlp_output = self.final_mlp_vars(torch.cat(pooled_var_beliefs, dim=1))
            #print(fac_mlp_output.shape, var_mlp_output.shape)
            #time.sleep(10000)
