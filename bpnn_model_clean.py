...
#                 self.shifted_relu1 = shift_func(ReLU(), shift=-50) #allow beliefs less than 0    
    ...
    #                 self.linear8.bias = torch.nn.Parameter(b8)
            ...
            if USE_MLP_DAMPING_VtoF:
                damping_mlp_test = False
                if damping_mlp_test:
                    self.linear11 = Linear(2*damping_scale*var_cardinality*belief_repeats, 2*damping_scale*var_cardinality*belief_repeats)
                    self.linear12 = Linear(2*damping_scale*var_cardinality*belief_repeats, damping_scale*var_cardinality*belief_repeats)
                else:
                ...
                if initialize_exact_BP:
                    if damping_mlp_test:
                        self.linear11.weight = torch.nn.Parameter(torch.eye(2*damping_scale*var_cardinality*belief_repeats))
                        self.linear11.bias = torch.nn.Parameter(torch.zeros(self.linear11.bias.shape))
                        layer_12_init = torch.zeros((2,4))
                        layer_12_init[0,0] = 1.0
                        layer_12_init[1,1] = 1.0
                        # print("layer_12_init:", layer_12_init)
                        # sleep(temp)
                        self.linear12.weight = torch.nn.Parameter(layer_12_init)
    ...
    def forward(self, factor_graph, prv_varToFactor_messages, prv_factorToVar_messages, prv_factor_beliefs, iter):
        ...
        return self.propagate(factor_graph=factor_graph, prv_varToFactor_messages=prv_varToFactor_messages,
                              prv_factorToVar_messages=prv_factorToVar_messages, prv_factor_beliefs=prv_factor_beliefs, iter=iter)
...
#                 print("torch.max(factorToVar_messages_exp):", torch.max(factorToVar_messages_exp))    
            ...
            # print("torch.max(prv_factorToVar_messages-factorToVar_messages):", torch.max(prv_factorToVar_messages-factorToVar_messages))
...
#                     # print("torch.max(torch.max(scale1),other=torch.tensor(1.0, device='cuda')):", torch.max(torch.max(scale1),other=torch.tensor(1.0, device='cuda')))
            ...
            # print("torch.max(prv_varToFactor_messages-varToFactor_messages):", torch.max(prv_varToFactor_messages-varToFactor_messages))
        ...
        if self.use_MLP1:
            if self.lne_mlp:
                ...
                if equivariance_type == 'varIdx_5d_permutation_equivariant':
                    ...
                    perm3 = torch.index_select(varToFactor_expandedMessages, dim=4, index=swap_idx_tensor)
                    perm4 = torch.index_select(perm1, dim=4, index=swap_idx_tensor)
                    perm5 = torch.index_select(perm2, dim=4, index=swap_idx_tensor)

                    out0 = self.mlp1(varToFactor_expandedMessages.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)

                    out1 = self.mlp1(perm1.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)
                    out1 = torch.index_select(out1, dim=2, index=swap_idx_tensor)

                    out2 = self.mlp1(perm2.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)
                    out2 = torch.index_select(out2, dim=3, index=swap_idx_tensor)

                    out3 = self.mlp1(perm3.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)
                    out3 = torch.index_select(out3, dim=4, index=swap_idx_tensor)

                    out4 = self.mlp1(perm4.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)
                    out4 = torch.index_select(out4, dim=4, index=swap_idx_tensor)
                    out4 = torch.index_select(out4, dim=2, index=swap_idx_tensor)

                    out5 = self.mlp1(perm5.view(varToFactor_expandedMessages_shape[0], -1)).view(varToFactor_expandedMessages_shape)
                    out5 = torch.index_select(out5, dim=4, index=swap_idx_tensor)
                    out5 = torch.index_select(out5, dim=3, index=swap_idx_tensor)
...
#         factor_beliefs = scatter_('add', varToFactor_expandedMessages, factor_graph.facToVar_edge_idx[0]) #for batching
        ...
        #                 factor_beliefs = self.mlp2(factor_beliefs.view(num_factors,-1)).view(factor_beliefs_shape)
                ...
                if equivariance_type == 'varIdx_5d_permutation_equivariant':
                    ...
                    perm3 = torch.index_select(factor_beliefs, dim=4, index=swap_idx_tensor)
                    perm4 = torch.index_select(perm1, dim=4, index=swap_idx_tensor)
                    perm5 = torch.index_select(perm2, dim=4, index=swap_idx_tensor)

                    out0 = self.mlp1(factor_beliefs.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)

                    out1 = self.mlp1(perm1.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)
                    out1 = torch.index_select(out1, dim=2, index=swap_idx_tensor)

                    out2 = self.mlp1(perm2.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)
                    out2 = torch.index_select(out2, dim=3, index=swap_idx_tensor)

                    out3 = self.mlp1(perm3.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)
                    out3 = torch.index_select(out3, dim=4, index=swap_idx_tensor)

                    out4 = self.mlp1(perm4.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)
                    out4 = torch.index_select(out4, dim=4, index=swap_idx_tensor)
                    out4 = torch.index_select(out4, dim=2, index=swap_idx_tensor)

                    out5 = self.mlp1(perm5.view(factor_beliefs_shape[0], -1)).view(factor_beliefs_shape)
                    out5 = torch.index_select(out5, dim=4, index=swap_idx_tensor)
                    out5 = torch.index_select(out5, dim=3, index=swap_idx_tensor)
...
#             print("self.alpha_FtoV_msg:", self.alpha_FtoV_msg)
            ...
            if self.USE_MLP_DAMPING_FtoV:
                ...
                if self.FC_damping_layer:
                    ...
                    if False:  #quick test where damping MLPs get the actual messages as input in addition to the message difference
                        # print("prv_factorToVar_messages.shape:", prv_factorToVar_messages.shape)
                ...
                # test_mask0 = torch.randint_like(factorToVar_messages, low=0, high=2)
                # # print("test_mask0", test_mask0)
                # # print("test_mask0.shape", test_mask0.shape)
                # # print("factorToVar_messages.shape", factorToVar_messages.shape)
                # factorToVar_messages = test_mask0*prv_factorToVar_messages +\
                #                        (1-test_mask0)*(self.alpha_damping_FtoV*factorToVar_messages + (1 - self.alpha_damping_FtoV)*prv_factorToVar_messages)
...
#         assert(False), "finish these todos"
        ...
        if self.learn_damping_coefficients:
            ...
            if self.USE_MLP_DAMPING_VtoF:
                ...
                if self.FC_damping_layer:
                    ...
                    if False:  #quick test where damping MLPs get the actual messages as input in addition to the message difference
                        # print("prv_factorToVar_messages.shape:", prv_factorToVar_messages.shape)
                        message_diff = prv_varToFactor_messages-varToFactor_messages
                        diff_cat_msgs = torch.cat([message_diff, varToFactor_messages], dim=-1)
                        zero_cat_msgs = torch.cat([torch.zeros_like(prv_varToFactor_messages), varToFactor_messages], dim=-1)
                        mlp_out = self.mlp_dampingVtoF(diff_cat_msgs)
                        # assert (mlp_out == message_diff).all(), (mlp_out, message_diff)
                        varToFactor_messages = varToFactor_messages + (1 - self.alpha_damping_VtoF)*(self.mlp_dampingVtoF(diff_cat_msgs)
                                                                                                   - self.mlp_dampingVtoF(zero_cat_msgs))
                        # print("factorToVar_messages.shape:", factorToVar_messages.shape)
                        # sleep(temp)
                    else:                    
                ...
                # test_mask0 = torch.randint_like(varToFactor_messages, low=0, high=2)
                # varToFactor_messages = test_mask0*prv_varToFactor_messages +\
                #     (1-test_mask0)*(self.alpha_damping_VtoF*varToFactor_messages + (1 - self.alpha_damping_VtoF)*prv_varToFactor_messages)
    ...
    #             print(varToFactor_messages)
        ...
        # print("final torch.max(prv_varToFactor_messages-varToFactor_messages):", torch.max(prv_varToFactor_messages-varToFactor_messages))
        # print()
